...
(function () {
  ...
  function init() {
    ...
    $(".nav-btn").on("click", function (e) {
      ...
      // 如果点击的是当前页面的导航，不做任何操作
      if (targetPage === currentPage) return;
      mySwiper.slideTo(targetPage.slice(1) - 2);
      currentPage = targetPage;
    });
    mySwiper.on("slideChange", function () {
      // 获取当前激活的slide
      const activeSlide = mySwiper.slides[mySwiper.activeIndex];
      const targetPage = $(activeSlide).attr("id");
      // 隐藏所有导航栏
      $(".navigate").addClass("hidden");
      // 显示当前页面的导航栏
      $(`#${targetPage} .navigate`).removeClass("hidden");
      // 更新当前页面ID
      currentPage = targetPage;
    });
  ...
  function removeTouchMove() {
    document.body.addEventListener("touchmove", removeDefaultEvt, {
      passive: false,
    });
  }
})();
